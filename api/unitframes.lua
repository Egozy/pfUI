...
function pfUI.uf:CreateUnitFrame(unit, id, config, tick)
  ...
  f.incHeal.texture = f.incHeal:CreateTexture(nil, "OVERLAY")
...
function pfUI.uf:RefreshUnitState(unit)
  ...
  --if unit.hp.bar.texture then unit.hp.bar.texture:SetAlpha(alpha < 1 and 0 or 1) end
  --if unit.power.bar.texture then unit.power.bar.texture:SetAlpha(alpha < 1 and 0 or 1) end
...
function pfUI.uf:RefreshUnit(unit, component)
  ...
  unit.hp.bar:SetMinMaxValues(0, hpmax)
